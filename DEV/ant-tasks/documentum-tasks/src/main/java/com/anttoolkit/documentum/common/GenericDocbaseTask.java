...
public abstract class GenericDocbaseTask
		extends GenericTask
{
	private boolean m_showObjectInfoBeforeIteration = false;
	private boolean m_showObjectInfoAfterIteration = false;
	private String m_batchSuccessIterationsProperty = null;
	private String m_batchFailedIterationsProperty = null;
	private String m_batchIterationsProperty = null;
			...
			throws BuildException
	{
		...
		try
		{
			...
			try
			{
				...
				{
					if (m_showObjectInfoBeforeIteration)
					{
					...
					if (m_showObjectInfoAfterIteration)
					{
			...
			{
				if (m_batchIterationsProperty != null)
				{
					this.getProject().setProperty(m_batchIterationsProperty, Integer.toString(iteration));
				}

				if (m_batchSuccessIterationsProperty != null)
				{
					this.getProject().setProperty(m_batchSuccessIterationsProperty, Integer.toString(successIterations));
				}

				if (m_batchFailedIterationsProperty != null)
				{
					this.getProject().setProperty(m_batchFailedIterationsProperty, Integer.toString(failedIterations));
				}
			}
		}
		finally
			...
			throws BuildException
	{
		...
		catch (Throwable e)
		{
			if (!failOnError())
			{
				log("Handled exception occured during execution of batch object, r_object_id=" + objectId + ", iteration=" + iteration);
