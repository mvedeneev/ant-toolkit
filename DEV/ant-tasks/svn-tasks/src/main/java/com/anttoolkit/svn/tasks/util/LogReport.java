...
public class LogReport
{
	...
	private List<LogEntry> m_logEntries = new LinkedList<LogEntry>();
	...
	public List<LogEntry> getOrderedLogEntries()
	{
		if (!m_isOrdered)
		{
			Collections.sort(m_logEntries, LogEntryComparator.instance());
			m_isOrdered = true;
		}
	...
	{
		if (m_logEntries == null || m_logEntries.isEmpty())
		{
			return;
		}

		int count = m_logEntries.size();
		int i = 0;

		while (i < count)
		{
			LogEntry logEntry = m_logEntries.get(i);
			logEntry.removeEntriesNotStartedWithPath(path);

			if (logEntry.isEmpty())
			{
				m_logEntries.remove(logEntry);
				count--;
				continue;
			}
	...
	public void removeLogEntriesWithRevisionLowerThan(int revision)
	{
		...
		while (i < count)
		{
			LogEntry entry = m_logEntries.get(i);
