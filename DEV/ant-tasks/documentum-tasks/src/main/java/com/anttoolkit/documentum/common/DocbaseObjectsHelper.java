...
public class DocbaseObjectsHelper
{
	...
	private static volatile Map<String, String> m_docbaseOwners = new HashMap<String, String>();
			...
			throws DfException
	{
		...
		synchronized (m_docbaseOwners)
		{
			if (m_docbaseOwners.containsKey(docbaseName))
			{
				return m_docbaseOwners.get(docbaseName);
			}

			String owner = DqlHelper.getStringParamFromFirstString(session, DQL_GET_DOCBASE_OWNER);
			m_docbaseOwners.put(docbaseName, owner);
	...
	public static String getAttributeValueAsString(IDfTypedObject obj, String attrName, String dateTimeFormat)
	{
		...
		{
			throw new BuildException("Failed to get datatype for attribute \"" +
					attrName + "\"\r\n" + e.toString());
		}
	}
